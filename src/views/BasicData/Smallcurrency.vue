<template>
    <div class="content">
        <PageTemplate :loading='loading' @init='init' @changePage='changePage' :list="list" @search='search' :columns='columns' :dataSource='dataSource' :pagination='pageParams'>
            <a-button size='small' @click="changeData({})">新增小币种</a-button>
        </PageTemplate>
    </div>
</template>
...
export default defineComponent({
    setup(){
        ...
        const list:Array<listProps> = reactive([
            {title:'小币种',value:'',placeholder:'小币种',name:'a-input',serve:'kind_children_name'},
            {title:'大币种',value:'',placeholder:'大币种',name:'a-input',serve:'kind_name'},
        ])
        ...
        const getData = async (params:any)=>{
            loading.value = true
            let res:any = await getSmallcurrency(params)
            res.status = 200
            loading.value = false
            if(res.status == '200'){
