<template>
    <div class="content">
        <PageTemplate :loading='loading' @init='init' @changePage='changePage' :list="list" @search='search' :columns='columns' :dataSource='dataSource' :pagination='pageParams'>
            <a-button size='small' @click="changeData({})">新增大币种</a-button>
        </PageTemplate>
    </div>
</template>
...
export default defineComponent({
    setup(){
        ...
        const list:Array<listProps> = reactive([
            {title:'钱币名称',value:'',placeholder:'朝代',name:'a-input',serve:'dynastyName'},
            {title:'类型',value:null,placeholder:'年份',name:'a-select',serve:'dynastySort',
                option:result,optionName:'name',optionValue:'value'},
            {title:'朝代',value:null,placeholder:'年份',name:'a-select',serve:'dynastySort',
                option:result,optionName:'name',optionValue:'value'},
            {title:'皇帝',value:'',placeholder:'朝代',name:'a-input',serve:'dynastyName'},
            {title:'标签',value:'',placeholder:'朝代',name:'a-input',serve:'dynastyName'},
        ])
        const columns:ColumnProps[] = reactive([
            {title:'ID',dataIndex:'id'},
            {title:'钱币名称',dataIndex:'coinKindName'},
        ...
        const getData = async (params:any)=>{
            loading.value = true
            let res:any = await getBigcurrency(params)
            res.status = 200
            loading.value = false
            const {status} = res
